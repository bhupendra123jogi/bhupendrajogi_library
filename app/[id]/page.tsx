...
function formatBookNameFromQuery(queryName: string, bookList: Book[]): string | null {
  const formattedQueryName = decodeURIComponent(queryName); // Decode the query name if needed

  const book = bookList.find((book) => book.name.toLowerCase() === formattedQueryName.toLowerCase());
...
export default function Page({ params }: { params: { id: string } }) {
  ...
  return (
    <div className="scroll-m-3">
      ...
      {params.id == "tos" ? (
      ...
      ) : books.list.find((book) => book.name == formatBookNameFromQuery(params.id, books.list)) != undefined ? (
        ...
        <div style={{
          ...
          <object
            data={books.list.find((book) => book.name == formatBookNameFromQuery(params.id, books.list))!.link}
          ...
          >
            <p>
              ...
              <a href={books.list.find(book => book.name == formatBookNameFromQuery(params.id, books.list))!.link}>
                to the PDF!
              </a>
            </p>
          </object>
